...
func TestVirtualModeInjectedReal(t *testing.T) {
    ...
    if err != nil { t.Fatalf("open injected real: %v", err) }
    ...
    if err != nil { t.Fatalf("read injected real: %v", err) }
    if string(data) != "HELLO" { t.Fatalf("unexpected content: %q", data) }
}
...
func TestVirtualModeVirtualization(t *testing.T) {
    ...
    if err != nil { t.Fatalf("open virtualized: %v", err) }

    // Must NOT be registered as real file
    if vfs.IsRealFile(realPath) { t.Fatalf("virtualized path should not be marked real") }

    // Read should be empty because we created an empty virtual file, not the underlying file content
    data, err := io.ReadAll(f)
    if err != nil && err != io.EOF { t.Fatalf("read virtualized: %v", err) }
    if len(data) != 0 { t.Fatalf("expected empty virtual content, got %q", data) }

    // Verify underlying file still has content (sanity check)
    diskData, err := os.ReadFile(realPath)
    if err != nil { t.Fatalf("read underlying real file: %v", err) }
    if string(diskData) != "SECRET" { t.Fatalf("unexpected underlying content: %q", diskData) }
}
...
func TestInternalUnauthorizedRealVirtualized(t *testing.T) {
    ...
    if err := os.WriteFile(target, []byte("DATA"), 0644); err != nil { t.Fatalf("create file: %v", err) }

    internalVFS := VFSWithOptions(false, false, nil)
    f, err := internalVFS.OpenFileSession(target, os.O_RDONLY, 0)
    if err != nil { t.Fatalf("internal open should virtualize, got error: %v", err) }
    if internalVFS.IsRealFile(target) { t.Fatalf("internal unauthorized path should not be real") }
    data2, _ := io.ReadAll(f)
    if len(data2) != 0 { t.Fatalf("expected empty virtualized content, got %q", data2) }
}
...
func TestAllowedInheritance(t *testing.T) {
    ...
    if err := os.WriteFile(path, []byte("PARENT"), 0644); err != nil { t.Fatalf("create parent file: %v", err) }

    top := VFSWithOptions(true, false, nil)
    if _, err := top.OpenFileSession(path, os.O_RDONLY, 0); err != nil { t.Fatalf("top open real: %v", err) }
    if !top.IsRealFile(path) { t.Fatalf("expected top to register real file") }

    child := VFSWithOptions(false, false, nil)
    child.InheritAllowedFiles(top)

    f, err := child.OpenFileSession(path, os.O_RDONLY, 0)
    if err != nil { t.Fatalf("child open inherited real: %v", err) }
    data, err := io.ReadAll(f)
    if err != nil { t.Fatalf("read child real: %v", err) }
    if len(data) == 0 { t.Fatalf("expected non-empty content from inherited real file") }
}
