...
func TestLLMQuotaProtocolIntegration(t *testing.T) {
	...
	// Inject mock LLM client to provide stats/quota path
	proxy.SetLLMClient(newMockLLMClient())
	// Register client and set up mock LLM client and context
	proxy.registerClient(proxy.clientID)
	// Prime stats and mark LLM context by performing a minimal chat call
	_ = proxy.handleLLMChat(true, 0, 1, 2, []byte("file.txt\nhello"))
	...
	// Verify quota data format (allow either shared-quota "used/total" or client-stats "weighted tokens used")
	if !(strings.Contains(response.Data, "weighted tokens used") || strings.Contains(response.Data, "/")) {
		t.Errorf("Expected quota info to include usage stats, got %q", response.Data)
	}
}
