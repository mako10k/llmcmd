...
// EnableLLMBroker wires up the LLM broker with given initial snapshot and config.
func (proxy *FSProxyManager) EnableLLMBroker(snapshot llm.QuotaSnapshot, cfg llm.BrokerConfig) {
	if proxy.llmClient == nil {
		log.Printf("FS Proxy: EnableLLMBroker called without llmClient; keeping disabled")
		return
	}
	prov := newOpenAIProvider(proxy.llmClient)
	proxy.broker = llm.NewBroker(prov, snapshot, cfg)
	proxy.useBroker = true
	log.Printf("FS Proxy: LLM broker enabled (session %s)", snapshot.SessionID)
}
...
func (proxy *FSProxyManager) executeLLMCmd(isTopLevel bool, inputFiles []string, prompt string, stdinFD, stdoutFD, stderrFD int) (map[string]interface{}, string, error) {
	...
	// Close write end immediately since we don't stream stdin in this path.
	// This prevents blocking reads waiting for EOF in executeInternalWithPipes.
	stdinW.Close()
...
func (proxy *FSProxyManager) executeInternalWithPipes(args []string, stdin *os.File, stdout, stderr *os.File, isTopLevel bool) executionResult {
	...
	// Build request common parts
	system := "You are llmcmd, a text processing assistant with secure tool access. Process the user's request and provide a clear, helpful response."
	...
	if proxy.useBroker && proxy.broker != nil {
		// Use broker (serialized, soft trim, quota accounting)
		bresp, err := proxy.broker.Chat(ctx, llm.ChatRequest{
			Model:       "gpt-4o-mini",
		...
		if err != nil {
			...
			if len(bresp.Messages) > 0 {
				respContent = bresp.Messages[0].Content
			}
			usage = openai.Usage{PromptTokens: bresp.Usage.PromptTokens, CompletionTokens: bresp.Usage.CompletionTokens, TotalTokens: bresp.Usage.TotalTokens}
		}
	} else {
		// Direct client path (legacy)
		messages := []openai.ChatMessage{
			{Role: "system", Content: system},
			{Role: "user", Content: user},
		}
		request := openai.ChatCompletionRequest{Model: "gpt-4o-mini", Messages: messages, MaxTokens: 2000, Temperature: 0.7}
	...
	if callErr != nil {
		log.Printf("FS Proxy: LLM API call failed: %v", callErr)
		return executionResult{err: fmt.Errorf("LLM API error: %v", callErr), quotaStatus: "unknown/5000 weighted tokens (API call failed)"}
	}

	llmResponse := respContent
	...
	quotaStatus := fmt.Sprintf("%.1f/5000 weighted tokens (%d input, %d output)",
		float64(usage.PromptTokens)+float64(usage.CompletionTokens)*4.0,
		usage.PromptTokens, usage.CompletionTokens)

	log.Printf("FS Proxy: LLM API call completed successfully - tokens used: %d", usage.TotalTokens)
...
func (proxy *FSProxyManager) handleLLMQuota() FSResponse {
	...
	// Be permissive for internal/test calls: if any quota source exists, allow
	if !proxy.isLLMExecutionContext() && proxy.broker == nil && proxy.quotaManager == nil && proxy.llmClient == nil {
		return FSResponse{Status: "ERROR", Data: "LLM quota access denied"}
	}

	// Get quota information from broker (preferred), shared quota manager, or LLM client
	var quotaInfo string
	if proxy.useBroker && proxy.broker != nil {
		snap := proxy.broker.QuotaGet()
		quotaInfo = fmt.Sprintf("%.4f remaining units (weights: in=%.2f, cached=%.2f, out=%.2f; window=%s; session=%s)",
			snap.RemainingUnits, snap.Weights.Input, snap.Weights.Cached, snap.Weights.Output, snap.Window, snap.SessionID)
	} else if proxy.quotaManager != nil {
