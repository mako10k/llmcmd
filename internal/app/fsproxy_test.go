...
func parseFSProxyMessage(input string) (FSProxyMessage, error) {
	...
	case "READ":
		...
		// Validate fileno and size
		if err := validateFileNoAndSize(msg.FileNo, msg.Size); err != nil {
			return FSProxyMessage{}, err
		}
	...
	case "CLOSE":
		...
		// Validate fileno
		if err := validateNumericField("fileno", msg.FileNo); err != nil {
			return FSProxyMessage{}, err
		}
...
// validateNumericField checks whether the provided string is numeric, returning a formatted FSProxyError if not.
func validateNumericField(name, value string) *FSProxyError {
	if !isNumeric(value) {
		return &FSProxyError{Message: "invalid " + name + ": " + value}
	}
	return nil
}

// validateFileNoAndSize validates both fileno and size fields in one pass.
func validateFileNoAndSize(fileNo, size string) *FSProxyError {
	if err := validateNumericField("fileno", fileNo); err != nil {
		return err
	}
	if err := validateNumericField("size", size); err != nil {
		return err
	}
	return nil
}
