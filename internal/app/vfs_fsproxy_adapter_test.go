...
func TestVFSFSProxyAdapter(t *testing.T) {
	...
	t.Run("OpenFile", func(t *testing.T) {
		openWriteAndCheck(t, adapter, "test.txt", []byte("Hello, World!"))
	})

	// Test CreateTemp
	t.Run("CreateTemp", func(t *testing.T) {
		_ = createTempWriteAndCheck(t, adapter, "testpattern", nil)
	})

	// Test ListFiles
	t.Run("ListFiles", func(t *testing.T) {
		listFilesContains(t, adapter) // just ensure non-empty
	})
