...
- **通信手段**: Unix socketpair/pipe（親プロセスのMUXが多重化・中継）
- **継承方法**: 子プロセスはFD 3でFS Proxyにアクセス（親のMUXが子ごとのFDを集約）
- **データ形式**: JSON（4バイトBE長プリフィックス）を一次形式。テキストコマンドは非推奨。
- **同期方式**: リクエスト/レスポンス同期通信（idによる多重化）
...
注意: ハンドル 0/1/2 は予約済み（stdin/stdout/stderr）であり、VFS サーバ（vfsd）では使用不可です。これらへの read/write/close は親プロセスのMUXがローカルに処理し、上流には転送しません。
