...
---
## 1. 目的と位置付け
`llmcmd` と `llmsh` は「LLM が安全な“閉じた計算環境”内で、ファイル加工・テキスト整形・自己再帰的分解を行いながら高品質出力を生成する」ための二形態インターフェースである。なお 2025-08-16 時点で `llmsh` は Rust 実装（`llmsh-rs/`）に一本化され、Go 版（`internal/llmsh/*`, `cmd/llmsh/*`）は廃止された。
