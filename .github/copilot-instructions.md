...
#### 保護対象
- **有料リクエストで生成されたコード**: OpenAI GPT-4やClaude等の有料APIで生成されたすべてのコード
- **実装完了コンポーネント**: 動作確認済み・テスト済みの実装
- **設計資料・ドキュメント**: 分析結果、設計文書、アーキテクチャ図
...
#### 判断基準
- **削除禁止**: 有料APIで生成された成果物は原則削除しない
- **保存優先**: 疑わしい場合は保存を選択
- **文書化**: 保存理由・将来活用可能性を必ず記録
...
**原則**: "When in doubt, preserve with WIP branch" - 疑問がある場合はWIPブランチで保存
